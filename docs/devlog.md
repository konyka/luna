...
辅助api和基础库
    ...
    lua标准库包括基础库、数学库、字符串库、UTF-8库、表操作库、输入输出库、操作系统库、包盒模块库、协程库、调试库。其中基础库直接以全局变量的形式提供，因此可以在lua脚本中直接使用print pairs等基础库中的函数。其他库则是以模块的形式提供，使用的时候需要带上具体模块的前缀，math。random（）

    lua标准库里面的函数完全是用lua api盒辅助api实现的，因此标准库的很多函数编写起来比较容易，只要照搬Lua芳芳实现里面的c语言代码，然后改成go语法就可以了。

辅助api    

    辅助api就是为了解决基础api的易用性而存在的，完全建立在基础api之上，对一些常用的操作进行了封装，也就是说，没有什么事辅助api能做，而基础api做不了的，只不过有这些操作，用辅助api更方便而已。

    和基础API类似，辅助api在lua官方实现里面也表现为一系列的类型和函数或者宏，它们都以luaL_开头。为了代码的可读性，采用和基础api一样的做法，吧这些函数整合到了一个接口中

    api/lua_auxlib.go，定义接口
...
包 模块
    ...
    标准库提供的常量或者函数都被封装在各自的表中，这些表由北赋值给全局变量，所以才能以类似math.random()这样的方式使用。确切的说，这些苦都是模块的形式提供的。

说明
    包package和模块module是模块化编程（modular programming）中的概念。在lua语言中，函数是代码重用的最小单位，把若干函数和县官的常量整合在一起，放到单独的命名空间中，就构成了模块，多个模块由可以进一步构成包。

    为了避免名称冲突，很多语言都会在语法层面对命名空间namespace提供支持。lua没有从语法层面上提供命名空间，不过使用表完全可以达到这个目的
    ...
    就定义了一个简单的模块，里面有两个函数，如果将其放到mymod。lua中，则可以通过dofile使用这个模块

    mymod = dofile("pathto/mymod.lua")
    mymod.foo()
    mymod.bar()

    这个方法虽然简单有效，不过问题也不少。比如，应该从哪里寻找定义模块的文件呢？
    如果需要多次加载模块该怎么处理呢？为了解决这些问题，lua 5.1 通过package标准库提供了一个统一的包和模块管理机制。

require（）函数

    该函数用于加载模块。使用模块名（字符串类型）为参数，返回加载后的模块，一般来说是一个表。为了让模块看起来更像是语法层面的支持，require（）函数是作为全局变量提供的。可以将上面的dofile（）替换为require（）函数：

    mymod = require "mymod" //省略了方法调用的圆括号
    mymod.foo()
    mymod.bar()


    需要注意的是，提供给require（）的是模块名，而不是文件名，因此不需要指令路径。.luag后缀也不需要。
    require（）函数究竟是怎么把模块名和具体的文件关联起来的？？？？？
    首先要说明的是，require（）在真正加载模块之前，会先去package.loaded表中根据名字查找模块，如果找到的话（不一定是表），就认为模块已经加载，直接返回模块即可。
    ...
    标准库都是以模块的形式提供的。lua release的解释器已经默认打开了全部的标准库---通过调用辅助api的OpenLibs（），因此这些库已经在package.loaded表中了。下面的命令可以验证：

    lua -e 'for k, v in pairs(package.loaded) do print(k, v) end'

    $lua -e 'for k, v in pairs(package.loaded) do print(k, v) end'
    io  table: 0x7f80cc4046c0
    utf8    table: 0x7f80cc405bb0
    coroutine   table: 0x7f80cc404210
    string  table: 0x7f80cc405420
    bit32   table: 0x7f80cc403f90
    debug   table: 0x7f80cc404fe0
    _G  table: 0x7f80cc402be0
    package table: 0x7f80cc4036d0
    table   table: 0x7f80cc403b10
    math    table: 0x7f80cc4056d0
    os  table: 0x7f80cc404130
    192:go konyka$ 

    如果某个模块已经加载，要想重新加载也简单，直接把它从package.loaded表中删除就可以了。简单暴力

搜索器
    如果某个模块还未曾加载过，那么require（）也不会亲自去加载，而是会请加载器Loader代劳。加载器实际上就是普通的函数。为了找到与模块对应的加载器，require（）需要先求助搜索器（searcher）。搜索器也是普通的函数，接收的唯一参数就是模块名，如果搜索器可以找到加载器，则返回加载器和一个额外值，该值会传递给加载器，否则，返回一个字符串，解释为什么搜索失败，或者直接返回nil

    搜索器可以有多个，按照顺序存储在package.serarchers数组中。lua官方实现，预定义了四个搜索器，按照顺序分别是preload搜索器、lua搜索器、c搜索器、croot搜索器。执行如下的命令可以列出这四个搜索器：

        lua -e 'for k, v in pairs(package.searchers) do print(k, v) end'

      $ lua -e 'for k, v in pairs(package.searchers) do print(k, v) end'
    1   function: 0x7fb839403d80
    2   function: 0x7fb839403db0
    3   function: 0x7fb839403de0
    4   function: 0x7fb839403e10  

    其中preload搜索器，只是简单的搜索package.preload表，看看能不能根据模块名从中找到加载器
    如果用lua实现preload加载器，代码类似：
    ...
    lua搜索器永固搜索lua模块加载器，这个搜索器试图从package.path变量指定的路径中找到一个lua文件，然后加载该文件，加载后的函数就是加载器。文件查找逻辑和package.searchpath()一致。

    如果使用lua编写lua搜索器，代码类似：
    ...
    C 、CRoot搜索器用于搜索C模块加载器，在这里就是go模块加载器。

package.path   
    package.path 变量提供的搜索路径决定了lua搜索器从哪里搜索lua库文件，下面的命令用来打印出这个路径： 
    ...
    搜索路径是由多个子路径构成的，默认按照分号分隔。package.searchpath（）负责将这个路径分解，并把问好用模块名替换，得到一系列真正的文件路径，然后返回实际存在的第一个文件路径，或者nil和错误信息。另外，模块名里面包含的点，还会被替换为平台相关的路径分隔符，比如windows为\、其他系统是/。

    lua -e "print(package.searchpath('foo.bar', package.path))"

    $ lua -e "print(package.searchpath('foo.bar', package.path))"
    nil 
        no file '/usr/local/share/lua/5.3/foo/bar.lua'
        no file '/usr/local/share/lua/5.3/foo/bar/init.lua'
        no file '/usr/local/lib/lua/5.3/foo/bar.lua'
        no file '/usr/local/lib/lua/5.3/foo/bar/init.lua'
        no file './foo/bar.lua'
        no file './foo/bar/init.lua'

    由于模块名可以用点号分隔，最后又被转换为目录结构等形式，所以模块之间可以形成一种树状的层次结构。
    可以认为a.b.c和a.b.d模块是a.b模块的子模块。一组相互关联的模块按照这种层次结构组织起来就构成了一个包。

    文件目录分隔符、搜索路径分隔符、模块名占位符（默认是问号）等记录在package.config变量（字符串，使用换行符进行分隔）中
    ...
    function require(modname)  
        ...
        for i, searcher in pairs(package.searchers) do
            local loader, extra = searcher(modname)
            if type(loader) =="funciton" then
